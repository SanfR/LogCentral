<?php
  ...
  class Database
  {
    ...
    /** Connect to a MySQL database to be able to use the methods below.
      */
    function Database($base, $server, $user, $pass)
    {
      $this->mtStart    = $this->getMicroTime();
      $this->nbQueries  = 0;
      $this->lastResult = NULL;
      $res=$this->connect($server,$user,$pass,$base, true);
      if(!$res)
     	 die('Server connexion not possible.');
    }
   ...
   function select_db($db='') {
    ...
    function query($query, $debug = -1)
    {
  	...
  	//check if the sql was ok 
	public function checkSQL($text =''){
		$error = '';
		
		$error = mysql_error();
    	
		if ( $error != ''){
			throw new Exception($text." Error in your sql - ".$error);
    	}
    ...
    function debugAndDie($query)
    {
    	...
    	$this->errorMessageDebug = mysql_error();
      	$this->debugQuery('', "Error");
    ...
    function debugQuery($query, $reason = "Debug")
    {
      ...
      $this->errorMessageDebug=' Reason: '.$reason.' query:'.htmlentities($query);
  ...
  }//INSERT INTO users( username, first_name, last_name, password, date, activationcode, active, email) VALUES 
  //( "Sand", , "Sand", , "Sand", , "845e62da6aa5f2f0e8332f2a62e189bd7a098fda7f4659d2b1214174f70f9553", , "NOW()", , "86d7786e4ec9e5534c69caaab4a754ff1e8c81e6", , "0", , "sandrine34@gmail.com", )
